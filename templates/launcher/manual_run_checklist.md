...
Do not pass the artifact record id to `ingest-gate-result --artifact`; that flag accepts only an explicitly promoted graph artifact id. Use separate gate/finding/status commands only for recovery or a deliberately partial update. Coordinator merge, release, or research-stage closeout uses one `coord handoff`; standalone full `validate`, `build`, and root `smoke` remain diagnostic.
