...
2. Read scoped context with `agent-session-context` or `option-workstream-context`.
3. Check for queued or running runs that reference the worktree, output root, cache root, GPU, port, PID, or model path.
4. Check the outer repo and any explicitly relevant nested repos for dirty state.
5. Classify the code changes:
...
6. Ingest or link useful evidence before cleanup.
7. Record findings and any follow-up work.
8. Close or move the assignment cursor when the current node is terminal.
9. Only then remove the worktree and clean up the temporary branch.
10. Run `validate`, `build`, and `smoke` after structured state changes.
...
P0 note: this policy documents the intended metadata shape. Until the corresponding CLI write support is implemented, do not assume commands such as `create-artifact`, `update-run`, or `complete-run` will persist nested retention fields. Preserve this information in launcher output, artifact manifests, review bundles, or human notes instead of hand-editing YAML.
...
P0 note: this is the intended run metadata shape. Until CLI support exists, record the same details in launcher output or run handoff artifacts rather than assuming `complete-run` can write nested `output_retention`.
