...
当 assignment 或目标 experiment 已有 active run 时，不得用新的 session 轮换 lease；`coord assign` 返回 `active_run_blocks_session` 和 `active_run:<run_id>` blocker。先按 receipt 查看 execution context，并继续或关闭现有 run。

`create_worktree: true` 后若提交 conflict，receipt 标记 `partial_success: true`；保留 worktree/recovery marker，解决 blocker 后以相同 request 和 `operation_id` 重试。
