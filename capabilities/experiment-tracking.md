...
- Artifact record 保存 payload hash、stable path、links 与 provenance；payload 本身位于 `artifacts/`。
...
Final payload 使用 `work_close_v1.evidence_inputs`；只有提前 durability 才使用 `work record`。成功 receipt 的 `additional_verification_required: false` 表示不再追加 validate、context、build 或 smoke。
