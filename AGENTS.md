...
After a small known-node YAML edit, run changed-scope verification first:
...
For manual file edits, use `--changed-file <root-relative-path>` or `--changed-files <path>...`. If changed-scope `validate` returns `fallback.used_full_validation: true`, follow `fallback.recommended_commands` to refresh `dashboards/validation_index.json`, then retry the changed-scope check. Do not run full `build` or root `smoke` after every worker edit.

Before coordinator merge, release, or final handoff, run the full gate:
